...
export default function Tab({ game, children, ...p }: TabProps) {
    ...
    return <label className={`w-full transition-all grid place-items-center hover:shadow-sm has-[input:checked]:shadow-sm cursor-pointer has-[input:checked]:cursor-default px-[20px] py-[12px] font-medium has-[input:checked]:font-semibold outline outline-transparent outline-4 -outline-offset-4 rounded-[16px] text-[20px] ${conditionalClassname}`}>
        {children}
