...
export default function Tab({ game, children, huge, ...p }: TabProps) {
    ...
    return <label className={`w-full transition-all grid place-items-center cursor-pointer has-[input:checked]:cursor-default px-[20px] text-hwh-white-text-dark has-[input:checked]:text-hwh-background-dark has-[input:checked]:font-bold rounded-[8px] ${conditionalClassname} ${huge ? "font-normal text-[32px] py-[40px]" : "font-medium text-[20px] py-[20px]"}`}>
        {children}
